...
num_fia <- current_certificates %>% filter(fellow_type == "Fellow FIA") %>% nrow()


```

Data as at: `r certificate_database$last_date %>% max()`
...
### current insurance certificates issued
```{r}
valueBox(
...
### new insurance certificates issued in last 30 days
```{r}
valueBox(
...
### insurance certificates expired in last 30 days
```{r}
valueBox(
...
### Number of insurance certificates with CERA
```{r}
gauge(
  num_cera, 
  min = 0,
...
### Number of insurance certificates with FIA
```{r}
gauge(
  num_fia, 
  min = 0,
